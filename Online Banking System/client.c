...
void module(int socket, int choice){
    switch(choice){
        case 1: customer_module(socket);
                break;
        case 2: employee_module(socket);
                 break;
        case 3: manager_module(socket);
                break;
        case 4: admin_module(socket);
                break;
        default:
...
void menu(int socket) {
    ...
    while (1) {
        ...
        if (choice == 5) break;  // Exit if the user chooses to
